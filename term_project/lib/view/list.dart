...
class _ListScreenState extends State<ListScreen> {
  ...
  void initState() {
    ...
    _setupRecordListener();
  }

  void _setupRecordListener() {
    ...
    if (user != null) {
      _subscription = FirebaseFirestore.instance
          ...
          .listen((snapshot) {
        records =
            snapshot.docs.map((doc) => MyRecord.fromJson(doc.data())).toList();
        setState(() {});
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    List<MyRecord> filteredRecords = _selectedTab == 'All'
        ? records
            .where((record) => record.username == _currentUsername)
            .toList()
        : records
            .where((record) =>
                record.dateTime ==
                    DateFormat('yyyy-MM-dd').format(DateTime.now()) &&
                record.username == _currentUsername)
            .toList();
      ...
      child: Scaffold(
        ...
        body: Consumer<ThemeProvider>(
          builder: (context, themeProvider, child) {
            Color shadowColor =
                themeProvider.isDarkTheme ? Colors.black54 : Colors.grey[400]!;
                          ...
                          return ListTile(
                            title: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                image: DecorationImage(
                                  image: filteredRecords[index]
                                          .foodImage
                                          .isNotEmpty
                                      ? NetworkImage(
                                          filteredRecords[index].foodImage)
                                      : AssetImage(
                                              'assets/placeholder_image.jpg')
                                          as ImageProvider, // Placeholder for empty image URLs
                                  fit: BoxFit.fill,
                                ),
                                boxShadow: [
                                  BoxShadow(
                            ...
                            onTap: () {
                              context.go(
                                  '/main/list/${filteredRecords[index].id}');
                            },
                          );
                        },
                      ),
