...
class _RecentPhotoState extends State<RecentPhoto> {
  ...
  Future<void> _loadRecentFoodItems() async {
    ...
    _allDailyRecords = allRecords
        .where(
            (record) => record.dateTime == today && record.username == username)
        .toList();
    ...
    setState(() {
      print(
          'Loaded ${_foodItems.length} items. More than 4 items: $_hasMoreThanFourItems');
    });
  }

  void _updateNutrition() {
    int totalCalories = _allDailyRecords.fold<int>(
        0, (sum, item) => sum + _parseInt(item.calories));
    double totalProtein = _allDailyRecords.fold<double>(
        0.0, (sum, item) => sum + _parseDouble(item.protein));
    double totalFat = _allDailyRecords.fold<double>(
        0.0, (sum, item) => sum + _parseDouble(item.fat));
    double totalCarbs = _allDailyRecords.fold<double>(
        0.0, (sum, item) => sum + _parseDouble(item.carbs));
                    ...
                    ElevatedButton.icon(
                      onPressed: () async {
                        ...
                        if (newRecord != null && mounted) {
                          ...
                          Provider.of<ImagesProvider>(context, listen: false)
                              .setImageUrl(newRecord.foodImage);
                        } else if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content:
                                    Text('Failed to take or upload picture')),
                          );
                        }
                      },
                      ...
                      style: ButtonStyle(
                        backgroundColor: MaterialStateProperty.all<Color>(
                            Color.fromARGB(255, 86, 188,
                                81)), // Background color is set to orange
                      ),
                    ),
                  ],
                ),
